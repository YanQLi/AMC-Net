...
class MRIReconNet(nn.Module):
    ...
    def forward(self, under_kspace_up, mask_up, under_img_down, mask_down, refer):
        ...
        plt.axis('off'), plt.xticks([]), plt.yticks([])#, plt.savefig("result/refer.png", bbox_inches='tight', pad_inches=0)  # plt.title('refer')
        plt.figure(13)
        plt.imshow(np.rot90(real.cpu().squeeze()), cmap='gray'),
        plt.axis('off'), plt.xticks([]), plt.yticks([])#, plt.savefig("result/before.png", bbox_inches='tight', pad_inches=0)  # plt.title('before')
        plt.figure(14)
        plt.imshow(np.rot90(real_change.cpu().detach().numpy().squeeze()), cmap='gray'),
        plt.axis('off'), plt.xticks([]), plt.yticks([])#, plt.savefig("result/after_0.35.png", bbox_inches='tight', pad_inches=0)  # plt.title('after')
        plt.show()'''
        ...
        up_fea_1, loss_layers_up = self.cnn1_up(up_x_1_nF, mask_up)     # 无多模态融合输入 up_x_1_nF / down_x_1_nF
        down_fea_1, loss_layers_down = self.cnn1_down(down_x_1_nF, mask_down)
        ...
        # output_up = self.fusion21(rec_up_fea_2, down_to_up_2)
        # output_down = self.fusion22(rec_down_fea_2, up_to_down_2)
